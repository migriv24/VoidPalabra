 ...
 * Implemented rather than vendored to keep the library zero-dependency; verified
 * against the FIPS vectors in tests/canonical_test.cpp. When Phase 4 brings
 * signatures this should become libsodium's crypto_hash_sha256, which Void
...
void sha256_block(std::uint32_t h[8], const std::uint8_t* p) {
    std::uint32_t w[64];
    for (int i = 0; i < 16; ++i)
...
Digest sha256(const void* data, std::size_t len) {
    ...
    for (std::size_t i = 0; i < full; ++i) sha256_block(h, p + i * 64);
    ...
    for (int i = 0; i < 8; ++i)
        tail[tail_len - 1 - i] = static_cast<std::uint8_t>((bits >> (i * 8)) & 0xFF);
    for (std::size_t off = 0; off < tail_len; off += 64) sha256_block(h, tail + off);
...
Digest sha256(const std::string& s) { return sha256(s.data(), s.size()); }
