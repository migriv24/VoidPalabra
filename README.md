...
**What is built**, as of 2026-09-20 — nine layers, each `status:current` in the
...
| [sync session](okf/concepts/sync-session.md) | the protocol as a pure state machine — frames and the time in, frames and events out. Converges under loss, duplication, reordering and partition; withholds private runes in every form; files by hash; presence; a stream envelope for TCP-like transports. Opens no socket |
| [concurrent structure](okf/concepts/concurrent-structure.md) | several people rewriting one graph, tree or sheet at once: declared link rules (equivalence, capacity, acyclic) checked after a merge, `FieldJoin::Latest` on a Lamport stamp, and who wrote each value |
...
cmake --build build
ctest --test-dir build      # 17 suites: property tests, leaks, embedding, a simulated network, 308 conformance vectors
```
